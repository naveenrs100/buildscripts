...
log "cmd: mvn -f "${projectLocation}/pom.xml" sonar:sonar "
mvn -f "${projectLocation}/pom.xml" sonar:sonar  >> ${logfile} 2>&1
STATUS=$?
echo $STATUS
if [ $STATUS -ne 0 ]; then
    log "SONAR Failed and email is sent...!"
    sendEmail "$emailid" "***FAILED***AUTO-SONAR-FAILED-FOR-${tempCom}" "${logfile}" "${logfile}"
   # delete_ws $repo_ws
    #logout_rtc
  #  usage "Error"
fi
...
echo $ProjectID
printf "ProjectID=$ProjectID" > ${INJECT_FILENAME}
